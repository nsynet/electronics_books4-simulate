...
#include "main.h"
#define SCK_LOW 	HAL_GPIO_WritePin(GPIOD,SCK_Pin,GPIO_PIN_RESET) 
#define SCK_HIGH 	HAL_GPIO_WritePin(GPIOD,SCK_Pin,GPIO_PIN_SET) 

#define CS_LOW 		HAL_GPIO_WritePin(GPIOD,CS_Pin,GPIO_PIN_RESET) 
#define CS_HIGH 	HAL_GPIO_WritePin(GPIOD,CS_Pin,GPIO_PIN_SET)  

#define SID_LOW 	HAL_GPIO_WritePin(GPIOD,SID_Pin,GPIO_PIN_RESET) 
#define SID_HIGH 	HAL_GPIO_WritePin(GPIOD,SID_Pin,GPIO_PIN_SET) 

#define RST_LOW 	HAL_GPIO_WritePin(GPIOD,RST_Pin,GPIO_PIN_RESET) 
#define RST_HIGH 	HAL_GPIO_WritePin(GPIOD,RST_Pin,GPIO_PIN_SET) 
...
void LCD12864_ShowStr(uint8_t row, uint8_t col, const uint8_t *puts)
{
	ST7920_SendCmd(0x30);      				//基本指令集合
	ST7920_SendCmd(0x0C);
	ST7920_SendCmd(AC_TABLE[row][col]);      //起始位置
	delay_1ms(10);
	while(*puts != '\0')   
	{
		if(col >= 8)            //判断换行
...
//			row = 0;      //一屏显示完,回到屏左上角
			return;
		ST7920_SendCmd(AC_TABLE[row][col]);
		ST7920_SendData(*puts);      
		puts++;
		ST7920_SendData( *puts);
		puts++;
		col++;
...
{	
	RST_LOW; 					// RESET=0
	delay_1ms(10);   // wait for 10ms
	RST_HIGH;
	delay_1ms(50);   //wait for >40 ms
	ST7920_SendCmd(0x30);  // 8bit mode
	delay_1us(110);  //  >100us delay
	ST7920_SendCmd(0x30);  // 8bit mode
	delay_1us(40);  // >37us delay
	ST7920_SendCmd(0x08);  // D=0, C=0, B=0
	delay_1us(110);  // >100us delay
	ST7920_SendCmd(0x01);  // clear screen
	delay_1ms(12);  // >10 ms delay
	ST7920_SendCmd(0x06);  // cursor increment right no shift
	delay_1ms(1);  // 1ms delay
