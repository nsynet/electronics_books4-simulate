...
/**检测从机给主机返回的应答脉冲。从机接收到主机的复位信号后，
		会在15~60us后给主机发一个应答脉冲****/
u8 DS18B20_Answer_Check(void)
{
	...
	while (DS18B20_DQ_IN&&delay1 < 100)
	{
		delay1++;
		delay_nus(1);
	}
	...
	if (delay1 >= 100)//Hu200
		...
		delay1 = 0;
	// 有应答脉冲，且存在时间不超过240us
	while (!DS18B20_DQ_IN&&delay1 < 240)
	{
		delay1++;
		delay_nus(1);
	}
