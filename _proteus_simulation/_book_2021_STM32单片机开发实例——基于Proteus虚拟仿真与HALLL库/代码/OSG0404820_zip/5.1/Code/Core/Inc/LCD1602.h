...
#define RS_DataR()         HAL_GPIO_WritePin(GPIOA,RS_Pin,GPIO_PIN_SET)   //选择数据寄存器
#define RS_InstructionR()  HAL_GPIO_WritePin(GPIOA,RS_Pin,GPIO_PIN_RESET) //选择指令寄存器
#define RW_Read()          HAL_GPIO_WritePin(GPIOA,RW_Pin,GPIO_PIN_SET)   //读操作
#define RW_Write()         HAL_GPIO_WritePin(GPIOA,RW_Pin,GPIO_PIN_RESET) //写操作
...
void DataDir(char dir)
{
	...
	HAL_GPIO_WritePin(GPIOC,
			D0_Pin|D1_Pin|D2_Pin|D3_Pin|D4_Pin|D5_Pin|D6_Pin|D7_Pin, GPIO_PIN_SET);
	GPIO_InitStruct.Pin =
...
void LCD_Write_Cmd(uint8_t cmd)
{
	DataDir('O');
	WriteData(cmd);
	RS_InstructionR();RW_Write();E_Rst();
	RS_InstructionR();RW_Write();E_Set();
	__NOP();
	E_Rst();
	LCD_Busy_Wait();
}
...
void LCD_Init()
{
	LCD_Write_Cmd(0x38);HAL_Delay(2);
	LCD_Write_Cmd(0x01);HAL_Delay(2);
	LCD_Write_Cmd(0x06);HAL_Delay(2);
	LCD_Write_Cmd(0x0c);HAL_Delay(2);
}
