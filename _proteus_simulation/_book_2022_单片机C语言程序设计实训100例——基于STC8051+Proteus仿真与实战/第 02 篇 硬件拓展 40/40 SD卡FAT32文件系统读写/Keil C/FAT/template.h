//-----------------------------------------------------------------
// 名称: 格式化扇区数据模板(包括：DBR、FSINFO、FAT、DA首扇区)
//-----------------------------------------------------------------
#ifdef FAT_FORMAT_FS

ROM_TYPE_u8 _dbr[512] = {
 
};

ROM_TYPE_u8 _fsinfo_1[4] =		//4~479 is all 0 FSINFO扇区数据的第一部分 以RRaA开始
{
	0x52, 0x52, 0x61, 0x41 
};

ROM_TYPE_u8 _fsinfo_2[28] =		//484~511 is all 0 FSINFO扇区数据的第二部分 以rrAa开始，以0x55AA结束
{
	0x72, 0x72, 0x41, 0x61, 0xB6, 0xF9, 0x03, 0x00, 0x03, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x55, 0xAA 
};

ROM_TYPE_u8 _1stsec[26] =			//26~511 is all 0 首目录的第一个扇区数据
{ 
	'S', 'T', 'C', '8', '0', '5', '1', 'F', 'S', '-', '-', 0x08, 0x00, 0x6A, 0x50, 0x78, 
	0x7B, 0x40, 0x7B, 0x40, 0x00, 0x00, 0x50, 0x78, 0x7B, 0x40
};

ROM_TYPE_u8 _fatsec[12] =			//12~511 is all 0 FAT表的第一个扇区数据
{ 
	0xF8, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x0F
};

#endif