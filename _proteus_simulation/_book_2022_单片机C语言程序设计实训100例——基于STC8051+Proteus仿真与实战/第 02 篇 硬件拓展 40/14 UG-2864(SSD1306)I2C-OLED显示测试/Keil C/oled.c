...
void OLED_Set_Pos(u8 x, u8 y) {
	Write_IIC_Com(0xB0+y);			//设置起始页为第y页
	Write_IIC_Com(((x&0xF0)>>4)|0x10);//设置起始列高地址
	Write_IIC_Com((x&0x0F));		//设置起始列低地址
}
//-----------------------------------------------------------------
// 开启OLED显示
//-----------------------------------------------------------------
...
void OLED_Clear() {
	u8 i,n;
	for(i = 0; i < 8; i++) {		//共8页
		Write_IIC_Com(0xB0+i);		//设置页地址（i = 0~7）
		Write_IIC_Com(0x00);		//设置列地址低字节
		Write_IIC_Com(0x10);		//设置列地址高字节
		for(n = 0; n < 128; n++) Write_IIC_Dat(0);//写字符0
	} 
}
//-----------------------------------------------------------------
// 开显示
...
void OLED_On() {
	...
	for(i = 0; i < 8; i++) {		//共8页
		...
		Write_IIC_Com(0x00);		//设置列地址低字节
		Write_IIC_Com(0x10);		//设置列地址高字节
		for(n = 0; n < 128; n++) Write_IIC_Dat(1);//显示1条线
	}
}
//-----------------------------------------------------------------
// 在指定位置显示一个字符
