...
u8 enc28j60ReadOp(u8 op, u8 address) {
	u8 dat1;
	CS = 0;	WriteByte(op | (address & ADDR_MASK));
	dat1 = ReadByte();	if(address & 0x80) 	dat1 = ReadByte();
	CS = 1;
	return(dat1);
}

void enc28j60WriteOp(u8 op, u8 address, u8 mydat) {
	CS = 0;
...
u16 enc28j60_read_phyreg(u8 address) {
	u16 mydat;
